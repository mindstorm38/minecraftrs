use super::{Layer, NewLayer, LayerRand, State, LayerData, ComputeLayer};
...
impl NewLayer for ZoomLayer {
    ...
    fn generate(&mut self, x: i32, z: i32, output: &mut LayerData, context: LayerContext) {
        ...
        let input = context.generate_size(0, x_half, z_half, x_size_half, z_size_half).unwrap();
        let fuzzy = self.fuzzy;
        let mut tmp = LayerData::new(x_size_rounded, z_size_rounded, State::Uninit);
