...
impl Layer for VoronoiLayer {
    ...
    fn generate(&mut self, x: i32, z: i32, output: &mut LayerData, ctx: LayerContext) {
        ...
        let input = ctx.borrow_parent(0).unwrap()
            .generate_size(x_new, z_new, x_size_new, z_size_new);
